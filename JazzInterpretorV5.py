...
def subprogramModule(xValue, label): 
    ...
    if xValue[0] == "call":
       if (len(VariablesA)>1):
        ...
        cpp.addToMain(xValue[1] + "(" + variablesA + ");")
...
while num < max(range(len(splitLines))) + 1:
    ...
    if splitLines[num][0] in stackManipulationArray:                                    #######STACK MANIPULATION ARRAY
        ...
        while(splitLines[num][0] != "end" and splitLines[num][0] !="return"):
            subprogramModule(splitLines[num], labelIndex)
            num = num + 1
